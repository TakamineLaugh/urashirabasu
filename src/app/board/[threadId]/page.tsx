...
export default function ThreadDetail({ params }: { params: Promise<{ threadId: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('posts').insert([{ thread_id: Number(threadId), name: submitName, content: content, }]);
    if (error) { console.error(error); alert('書き込みに失敗しました'); } 
    else { setContent(''); await fetchData(); }
